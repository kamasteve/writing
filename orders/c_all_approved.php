...
<table width="100%" border="0">
  <tr valign="top">
    <td><div  style=" float:left;"class="titlehdr"><strong>My Approved Orders</strong></div></td>
    <td><div style="float:right;"><?php include 'gmttime.php'; ?></div></td>
  </tr>
</table>
      <?php 
	  $sql_finished = "select * from orders WHERE status ='Apv' and client_id='$_SESSION[id]' ORDER BY order_no DESC";
	  ...
	  ?>
		...
		<thead>
          <tr> 
			<th> <div align="center"><strong>Order ID</strong></div></th>
            ...
            <th ><div align="center"><strong>Words</strong></div></th>
	  ...
	  $usnames = "select  firstname, lastname, username, id from mu_members WHERE id ='$asgnd_id'";
...
<td class="order_borders" ><div align="center"> <a href="order-page.php?id=<?php echo $rrows['order_no'];?>&cid=<?php echo $rrows['client_id']; ?>&sts=<?php echo $rrows['status']; ?>&writer_ids=<?php echo $asgnd_id ;?>"><?php echo $rrows['order_no']; ?> </a></div></td>
			...
			<?php $uname = $rows_usnames['username'];
echo '<font size="1">Writer ID : '.$rows_usnames['id'] . '</font>' ; ?>
			</div></td>
...
echo $rrows['track_order_id'];
 ?>
            ...
            <td class="order_borders"><div align="center">
              <?php echo number_format($rrows['client_cost']) ; ?> &nbsp; ( <?php  echo $rrows['curr'] ?>)</div></td>
          </tr>
	          <?php  } ?>
	  </tbody>
        </table><br>
<?php 
	  $orders_done_costs = "select  sum( client_cost ) AS totalcosts  from orders  WHERE status ='Apv'";
	  ...
	  <br><br>
	  <div style="float:right; margin-right:1px; margin-top:18px;">Total: <span class="price_f"> <?php echo   $curr_symbol .' ' . number_format($total_price_) ; ?></span>		        </div>
    </td>
  </tr>
</table>
</div>
