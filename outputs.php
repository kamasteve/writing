<?php
//Urgency
date_default_timezone_set('GMT');

date_default_timezone_get();

$deadline= $data['urgency'];

$tnow = date('Y-m-d H:i:s');
$ftime = $deadline + time() ;

$futureTime = date("Y-m-d H:i:s", $ftime) ; 

$dateTime = new DateTime($futureTime); 
$f_timestamp = $dateTime->format('U'); 

$urg = $f_timestamp;


//academic level
$levels=$data['academic_level'];
			if($levels==0)
			 {
			 $acdmcLevel='High School';
			 }
			 if($levels==1)
			 {
			 $acdmcLevel='College';
			 }
			 if($levels==2)
			 {
			 $acdmcLevel='Undegraduate';
			 }
			 if($levels==3)
			 {
			 $acdmcLevel='Master';
			 }
			 if($levels==4)
			 {
			 $acdmcLevel='Ph. D';
			 }
//document type
$document_Type=$data['doctype'];
			if($document_Type==0)
			 {
			 $documentType='Dissertation';
			 }
			 if($document_Type==1)
			 {
			 $documentType='Thesis';
			 }
			 if($document_Type==2)
			 {
			 $documentType='Research Proposal';
			 }
			 if($document_Type==3)
			 {
			 $documentType='Essay';
			 }
			 if($document_Type==4)
			 {
			 $documentType='Term Paper';
			 }
			 if($document_Type==5)
			 {
			 $documentType='Article';
			 }
			 if($document_Type==16)
			 {
			 $documentType='Dissertation Chapter - Hypothesis';
			 }
			 if($document_Type==17)
			 {
			 $documentType='Dissertation Chapter - Conclusion Chapter';
			 }
			 if($document_Type==21)
			 {
			 $documentType='Thesis Proposal';
			 }
			 if($document_Type==41)
			 {
			 $documentType='Research Paper';
			 }
			 if($document_Type==51)
			 {
			 $documentType='Personal Statement';
			 }
			 if($document_Type==52)
			 {
			 $documentType='Annotated Bibliography';
			 }
			 if($document_Type==53)
			 {
			 $documentType='Coursework GCSE';
			 }
			 if($document_Type==54)
			 {
			 $documentType='Case Study';
			 }
			 if($document_Type==31)
			 {
			 $documentType='Admission/Application Essay';
			 }
			 if($document_Type==56)
			 {
			 $documentType='Coursework AS and A-Level';
			 }
			 if($document_Type==57)
			 {
			 $documentType='Assignment';
			 }
			 if($document_Type==63)
			 {
			 $documentType='Speech/Presentation';
			 }
			 if($document_Type==64)
			 {
			 $documentType='Statistics Project';
			 }
			 if($document_Type==82)
			 {
			 $documentType='Proofreading';
			 }
			 if($document_Type==65)
			 {
			 $documentType='Book Report/Review';
			 }
			 if($document_Type==62)
			 {
			 $documentType='Math Problem';
			 }
			 if($document_Type==61)
			 {
			 $documentType='Lab Report';
			 }
			 if($document_Type==66)
			 {
			 $documentType='Movie Review';
			 }
			 if($document_Type==81)
			 {
			 $documentType='Formatting';
			 }
			 if($document_Type==9)
			 {
			 $documentType='PowerPoint Presentation';
			 }
			 if($document_Type==10)
			 {
			 $documentType='Dissertation Chapter - Abstract';
			 }
			 if($document_Type==11)
			 {
			 $documentType='Dissertation Chapter - Introduction Chapter';
			 }
			 if($document_Type==12)
			 {
			 $documentType='Dissertation Chapter - Literature Review';
			 }
			 if($document_Type==13)
			 {
			 $documentType='Dissertation Chapter - Methodology';
			 }
			 if($document_Type==14)
			 {
			 $documentType='Dissertation Chapter - Results';
			 }
			 if($document_Type==15)
			 {
			 $documentType='PDissertation Chapter - Discussion';
			 }
//citation
$spacing= $data['o_interval'];
			if($spacing==1)
			 {
			 $word_spacing='Single Spacing';
			 }
			 if($spacing==0)
			 {
			 $word_spacing='Double Spacing';
			 }
            
  ?>